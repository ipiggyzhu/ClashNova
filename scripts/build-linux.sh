...
#   (libwebkit2gtk-4.1-dev 会自动带上 gtk-3-dev / libsoup-3.0-dev / jscore)
#
# 说明:
...
if command -v pkg-config >/dev/null 2>&1; then
  for pc in webkit2gtk-4.1 dbus-1 librsvg-2.0; do
...
if [ "${#missing[@]}" -ne 0 ]; then
  ...
  echo "请先运行:" >&2
  echo "  sudo apt-get update && sudo apt-get install -y pkg-config libwebkit2gtk-4.1-dev libdbus-1-dev librsvg2-dev" >&2
  exit 1
fi
